...
# Шпаргалку по работе с командами git:т
git add-добавляет изменения из файла в специальную область(но не сохранияет)<br>
git commit-сохраняет файлы<br>
git push-отправялет сохраненные коммиты из компьютера в репозиторий<br>
git status-показывает текущее состояние рабочей папки<br>
git clone-скачивает копию репозитория на компьютер<br>
